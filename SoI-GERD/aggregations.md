...
**Note:** this aggregation method scores the best solution in each generation as `-100`, making the score relative rather than absolute. As a result, scores cannot be directly compared acro
ss generations to determine whether the population is actually improving, even if it is. This can interfere with the early-stopping mechanism based on `max_stall`, sometimes triggering a "too fast convergence" warning. However, as long as the other monitored parameters (mean and diversity) remain stable across the last generations, this warning does not necessarily indicate a problem and can be disregarded.
